...
typedef struct wiimote_t {
  ...
  WCONST byte event_buf[MAX_PAYLOAD];    /**< read buffer              */ 
	/** @} */
#endif
...
#define WIIUSE_HAS_OUTPUT_REDIRECTION
  ...
  WIIUSE_EXPORT extern void wiiuse_expansion_stream(struct wiimote_t* wm, int status);
  ...
  WIIUSE_EXPORT extern int wiiuse_read_data_cb(struct wiimote_t* wm, wiiuse_read_cb read_cb, byte* buffer, unsigned int addr, uint16_t len);
  WIIUSE_EXPORT extern int wiiuse_write_data_cb(struct wiimote_t *wm, unsigned int addr, byte *data, byte len, wiiuse_write_cb write_cb);
